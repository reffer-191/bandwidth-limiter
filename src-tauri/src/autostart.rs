...
pub fn is_enabled() -> bool {
    schtasks(&["/Query", "/TN", TASK_NAME]).map(|(ok, _)| ok).unwrap_or(false)
}
...
pub fn set_enabled(enabled: bool) -> Result<bool, String> {
    if enabled {
        ...
        if !ok {
            return Err(format!("No se pudo crear la tarea de inicio: {}", text.trim()));
        }
    } else {
        let (ok, text) = schtasks(&["/Delete", "/F", "/TN", TASK_NAME])?;
        if !ok && !text.contains("ERROR: The system cannot find") && is_enabled() {
            return Err(format!("No se pudo eliminar la tarea de inicio: {}", text.trim()));
        }
    }
    Ok(is_enabled())
}
