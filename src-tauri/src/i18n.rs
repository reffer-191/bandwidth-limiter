...
pub fn tr(lang: Lang, key: &str) -> &'static str {
    let (es, en): (&'static str, &'static str) = match key {
        ...
        "tray.profile" => ("Perfil", "Profile"),
        "rule.pc" => ("Todo el equipo", "Whole computer"),
        "rule.hotspot" => ("Hotspot", "Hotspot"),
        "notif.newApp" => ("Nueva aplicación en la red", "New application on the network"),
        "notif.newDevice" => ("Nuevo dispositivo en el hotspot", "New device on the hotspot"),
        "notif.schedule.on" => ("Regla activada por horario", "Rule activated by schedule"),
        "notif.schedule.off" => ("Regla desactivada por horario", "Rule deactivated by schedule"),
        "notif.quota" => ("Cuota agotada", "Quota reached"),
        "notif.quota.block" => ("{used} de {quota} consumidos: el tráfico queda bloqueado hasta el próximo periodo.", "{used} of {quota} used: traffic is blocked until the next period."),
        "notif.quota.notify" => ("{used} de {quota} consumidos.", "{used} of {quota} used."),
        "notif.test" => ("Así se verán los avisos de Bandwidth Limiter.", "This is how Bandwidth Limiter notifications look."),
        "io.read" => ("No se pudo leer el fichero", "Could not read the file"),
        "io.write" => ("No se pudo escribir el fichero", "Could not write the file"),
        "io.format" => ("El fichero no contiene reglas de Bandwidth Limiter", "The file does not contain Bandwidth Limiter rules"),
        "dlg.rules" => ("Reglas de Bandwidth Limiter", "Bandwidth Limiter rules"),
