...
| `Bandwidth.Limiter_{version}_x64_offline-setup.exe` | Installer with the WebView2 runtime embedded (~210 MB) for PCs without Internet. | Instalador con el runtime WebView2 embebido (~210 MB) para equipos sin Internet. |
| `BandwidthLimiter-{version}-portable.zip` | Portable: unzip and run; needs WebView2 already installed. | Portable: descomprimir y ejecutar; requiere WebView2 ya instalado. |
